...
export class Signaling extends EventTarget {
    ...
    url(method) {
        return window.location.origin + "/signaling/" + method;
    }
    ...
    async loopGetConnection() {
        ...
        while (this.running) {
            ...
            const deleteConnection = new Set(
                [...currentConnections].filter((e) => !newSet.has(e))
            );

            deleteConnection.forEach((connection) => {
                this.dispatchEvent(
                    new CustomEvent("disconnect", {
                        detail: { connectionId: connection },
                    })
                );
        ...
        while (this.running) {
            ...
            if (candidates.length > 0) {
                ...
                for (let candidate of candidates[0].candidates) {
                    ...
                    this.dispatchEvent(
                        new CustomEvent("candidate", { detail: dispatch })
                    );
                }
            }
